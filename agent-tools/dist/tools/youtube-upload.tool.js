...
import { z } from 'zod';
let YouTubeUploadTool = class YouTubeUploadTool extends ToolContext {
    constructor() {
        super(...arguments);
        this.TOKEN_FILE = path.join(process.cwd(), 'youtube-token.json');
        this.CLIENT_SECRET_FILE = path.join(process.cwd(), 'client_secret.json');
    }
    ...
    async execute(input) {
        ...
        const thumbnailResponse = await fetch(`https://www.googleapis.com/upload/youtube/v3/thumbnails/set?videoId=${videoId}`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${credentials.access_token}`,
...
export default YouTubeUploadTool;
//# sourceMappingURL=youtube-upload.tool.js.map