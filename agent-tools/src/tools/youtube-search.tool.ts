...
export default class YouTubeSearchTool extends ToolContext {
    private parseSubFilter(subscribers?: string): { min?: number; max?: number } {
    ...
    private toNumber(val: number, unit?: string): number {
        if (!unit) return val;
        const multipliers: Record<string, number> = { k: 1000, m: 1000000, b: 1000000000 };
        return val * (multipliers[unit.toLowerCase()] || 1);
    }

    private formatSubs(count: number): string {
        if (count >= 1000000) return `${(count / 1000000).toFixed(1)}M`;
        if (count >= 1000) return `${(count / 1000).toFixed(1)}K`;
        return count.toString();
    }

    async execute(input: { category: string; subscribers?: string; location?: string }) {
        ...
        if (input.location) url.searchParams.append('regionCode', input.location.toUpperCase());
        ...
        let results = statsData.items?.map((item: any) => ({
            channelId: item.id,
        ...
        if (subFilter.min) results = results.filter((r: any) => r._raw >= subFilter.min!);
        if (subFilter.max) results = results.filter((r: any) => r._raw <= subFilter.max!);

        return { results: results.slice(0, 10).map(({ _raw, ...r }: any) => r) };
    }
}
